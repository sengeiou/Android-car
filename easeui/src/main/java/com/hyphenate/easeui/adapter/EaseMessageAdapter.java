...
public class EaseMessageAdapter extends BaseAdapter {
    ...
    java.util.List<EMMessage> myEmMessageList = new ArrayList<>();
    Handler handler = new Handler() {
        private void refreshList() {
            if (EaseConstant.IS_BUYCAR_TYPE) {
                try {
                    myEmMessageList.clear();
                    java.util.List<EMMessage> var = conversation.getAllMessages();
                    for (EMMessage msg : var) {
                        JSONObject oderObj = null;
                        try {
                            oderObj = msg.getJSONObjectAttribute("data");
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
                        if (oderObj != null) {
                            String odreId = oderObj.optString("order_id");
//                            Log.d("easechat", "easechat origin" + odreId);
//                            Log.d("easechat", "easechat origin" + msg.getBody());
                            if ((!TextUtils.isEmpty(odreId)) && EaseConstant.BUYCAR_ORDERID.equals(odreId)) {
                                if (EaseConstant.IS_BUYCAR_MANAGE) {
                                    String type = oderObj.optString("type");
                                ...
                                myEmMessageList.add(msg);
                            }
                        }
                    }
//                    for (EMMessage msg : myEmMessageList) {
//                        JSONObject oderObj = null;
//                        try {
//                            oderObj = msg.getJSONObjectAttribute("data");
//                            String odreId = oderObj.optString("order_id");
//                            Log.d("easechat", "easechat odreId" + odreId);
//                            Log.d("easechat", "easechat odreId" + msg.getBody());
//                        } catch (HyphenateException e) {
//                            e.printStackTrace();
//                        }
//
//                    }
                    messages = myEmMessageList.toArray(new EMMessage[myEmMessageList.size()]);
                } catch (Exception e) {
                    e.printStackTrace();
                }
            } else if (EaseConstant.IS_PRIVATE_ETTER) {
                myEmMessageList.clear();
                java.util.List<EMMessage> var = conversation.getAllMessages();
                for (EMMessage msg : var) {
                    String oderObj = null;
                    try {
                        oderObj = msg.getStringAttribute("model");
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                    if (oderObj != null) {
                        if ("privateLetter".equals(oderObj)) {
                            myEmMessageList.add(msg);
                        }
                    }
                }
...
//			conversation.markMessageAsRead();
//			conversation.markAllMessagesAsRead();
        ...
        public void handleMessage(android.os.Message message) {
            switch (message.what) {
                case HANDLER_MESSAGE_REFRESH_LIST:
                    EMLog.d("easechat", "easechat handle HANDLER_MESSAGE_REFRESH_LIST");
                    refreshList();
                    break;
                case HANDLER_MESSAGE_SELECT_LAST:
                    ...
                    EMLog.d("easechat", "easechat handle HANDLER_MESSAGE_SELECT_LAST");
                    break;
                case HANDLER_MESSAGE_SEEK_TO:
                    int position = message.arg1;
                    listView.setSelection(position);
                    EMLog.d("easechat", "easechat handle HANDLER_MESSAGE_SEEK_TO");
                    break;
                default:
                    break;
            }
    ...
    public int getCount() {
        ...
        EMLog.d("easechat", "easechat count==" + count);
        return count;
    }
