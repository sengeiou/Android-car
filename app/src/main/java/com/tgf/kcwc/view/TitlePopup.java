...
public class TitlePopup extends PopupWindow {
    ...
    //定义弹窗子类项列表
    private ArrayList<MorePopupwindowBean> mActionItems = new ArrayList<MorePopupwindowBean>();
    ...
    private void populateActions() {
        ...
        mListView.setAdapter(new BaseAdapter() {
            ...
            public View getView(int position, View convertView, ViewGroup parent) {
                ...
                MorePopupwindowBean item = mActionItems.get(position);
                ...
                //设置在文字的左边放一个图标
                if (item.icon != 0) {
                    textView.setCompoundDrawablesWithIntrinsicBounds(mContext.getResources().getDrawable(item.icon), null, null, null);
                }
     ...
     * 添加子类项
     */
    public void addAction(ArrayList<MorePopupwindowBean> action) {
        if (action != null) {
            mActionItems.addAll(action);
            mIsDirty = true;
        }
    }
     ...
     * 根据位置得到子类项
     */
    public MorePopupwindowBean getAction(int position) {
    ...
    public static interface OnItemOnClickListener {
        public void onItemClick(MorePopupwindowBean item, int position);
    }
}