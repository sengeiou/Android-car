 ...
 */
public class ExBackPagerSlidingTabStrip extends HorizontalScrollView {
    ...
    public ExBackPagerSlidingTabStrip(Context context) {
        this(context, null);
    }

    public ExBackPagerSlidingTabStrip(Context context, AttributeSet attrs) {
    ...
    public ExBackPagerSlidingTabStrip(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
        this.context = context;
        setFillViewport(true);
        setWillNotDraw(false);// 防止onDraw方法不执行

        View root = LayoutInflater.from(context).inflate(R.layout.ex_back_tagstrip_layout, null,
                false);
        tabsContainer = (LinearLayout) root.findViewById(R.id.tab_container);
        addView(root);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        System.out.println("tabCount:" + tabsContainer.getWidth() + ",tabLeft:" + tabsContainer.getLeft() + "," + tabsContainer.getRight());
        int tabsWidth = tabsContainer.getWidth();
        //计算指示器的坐标值，能够自适应
        drawLeft = tabsContainer.getLeft() + tabsWidth / (2 * tabCount) * (2 * currentPosition + 1) - tabsWidth / (4 * tabCount);
        drawRight = drawLeft + BitmapUtils.dp2px(context, 35);
        ...
        canvas.drawRect(drawLeft, height - indicatorHeight, drawRight, height, rectPaint);
        // draw divider
