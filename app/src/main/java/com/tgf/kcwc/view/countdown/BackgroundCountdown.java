package com.tgf.kcwc.view.countdown;
...
class BackgroundCountdown extends BaseCountdown {
    ...
    public void initStyleAttr(Context context, TypedArray ta) {
        ...
        isShowTimeBgDivisionLine = ta.getBoolean(R.styleable.CountdownView_isShowTimeBgDivisionLine,
            true);
        mTimeBgDivisionLineColor = ta.getColor(R.styleable.CountdownView_timeBgDivisionLineColor,
            Color.parseColor("#30FFFFFF"));
        mTimeBgDivisionLineSize = ta.getDimension(R.styleable.CountdownView_timeBgDivisionLineSize,
            Utils.dp2px(context, DEFAULT_TIME_BG_DIVISION_LINE_SIZE));
        mTimeBgSize = ta.getDimension(R.styleable.CountdownView_timeBgSize, 0);
        mDefSetTimeBgSize = mTimeBgSize;
        mTimeBgBorderSize = ta.getDimension(R.styleable.CountdownView_timeBgBorderSize,
            Utils.dp2px(context, DEFAULT_TIME_BG_BORDER_SIZE));
    ...
    private void initTimeBgRect(float topPaddingSize) {
        ...
        if (isShowDay) {
            ...
            if (isShowTimeBgBorder) {
                mDayBgBorderRectF = new RectF(mLeftPaddingSize, topPaddingSize,
                    mLeftPaddingSize + mDayTimeBgWidth + (mTimeBgBorderSize * 2),
                    topPaddingSize + mTimeBgSize + (mTimeBgBorderSize * 2));
                mDayBgRectF = new RectF(mLeftPaddingSize + mTimeBgBorderSize,
                ...
                mDayBgRectF = new RectF(mLeftPaddingSize, topPaddingSize,
                    mLeftPaddingSize + mDayTimeBgWidth, topPaddingSize + mTimeBgSize);
            }
            // hour left point
        ...
        if (isShowHour) {
            ...
            if (isShowTimeBgBorder) {
                mHourBgBorderRectF = new RectF(mHourLeft, topPaddingSize,
                    mHourLeft + mTimeBgSize + (mTimeBgBorderSize * 2),
                    topPaddingSize + mTimeBgSize + (mTimeBgBorderSize * 2));
                mHourBgRectF = new RectF(mHourLeft + mTimeBgBorderSize,
                    topPaddingSize + mTimeBgBorderSize, mHourLeft + mTimeBgSize + mTimeBgBorderSize,
                    topPaddingSize + mTimeBgSize + mTimeBgBorderSize);
            } else {
        ...
        if (isShowMinute) {
            ...
            if (isShowTimeBgBorder) {
                mMinuteBgBorderRectF = new RectF(mMinuteLeft, topPaddingSize,
                    mMinuteLeft + mTimeBgSize + (mTimeBgBorderSize * 2),
                    topPaddingSize + mTimeBgSize + (mTimeBgBorderSize * 2));
                mMinuteBgRectF = new RectF(mMinuteLeft + mTimeBgBorderSize,
                    topPaddingSize + mTimeBgBorderSize,
                    mMinuteLeft + mTimeBgSize + mTimeBgBorderSize,
                    topPaddingSize + mTimeBgSize + mTimeBgBorderSize);
            } else {
                mMinuteBgRectF = new RectF(mMinuteLeft, topPaddingSize, mMinuteLeft + mTimeBgSize,
                    topPaddingSize + mTimeBgSize);
            }
            // second left point
        ...
        if (isShowSecond) {
            ...
            if (isShowTimeBgBorder) {
                mSecondBgBorderRectF = new RectF(mSecondLeft, topPaddingSize,
                    mSecondLeft + mTimeBgSize + (mTimeBgBorderSize * 2),
                    topPaddingSize + mTimeBgSize + (mTimeBgBorderSize * 2));
                mSecondBgRectF = new RectF(mSecondLeft + mTimeBgBorderSize,
                    topPaddingSize + mTimeBgBorderSize,
                    mSecondLeft + mTimeBgSize + mTimeBgBorderSize,
                    topPaddingSize + mTimeBgSize + mTimeBgBorderSize);
            } else {
                mSecondBgRectF = new RectF(mSecondLeft, topPaddingSize, mSecondLeft + mTimeBgSize,
                    topPaddingSize + mTimeBgSize);
            }
    ...
    private void initHasBackgroundTextBaseY(RectF rectF) {
        ...
        mTimeTextBaseY = rectF.top
                         + (rectF.bottom - rectF.top - timeFontMetrics.bottom + timeFontMetrics.top)
                           / 2
                         - timeFontMetrics.top - mTimeTextBottom;
        // initialize background division line y point
        mTimeBgDivisionLineYPos = rectF.centerY() + (mTimeBgDivisionLineSize == Utils
            .dp2px(mContext, DEFAULT_TIME_BG_DIVISION_LINE_SIZE) ? mTimeBgDivisionLineSize
     ...
     */
    private float initTimeTextBaselineAndTimeBgTopPadding(int viewHeight, int viewPaddingTop,
                                                          int viewPaddingBottom,
                                                          int contentAllHeight) {
    ...
    @Override
    public void onMeasure(View v, int viewWidth, int viewHeight, int allContentWidth,
                          int allContentHeight) {
        float retTopPaddingSize = initTimeTextBaselineAndTimeBgTopPadding(viewHeight,
            v.getPaddingTop(), v.getPaddingBottom(), allContentHeight);
        mLeftPaddingSize = v.getPaddingLeft() == v.getPaddingRight()
            ? (viewWidth - allContentWidth) / 2 : v.getPaddingLeft();
    ...
    public void onDraw(Canvas canvas) {
        ...
        if (isShowDay) {
            ...
            if (isDrawBg) {
                ...
                if (isShowTimeBgDivisionLine) {
                    ...
                    canvas.drawLine(mLeftPaddingSize + mTimeBgBorderSize, mTimeBgDivisionLineYPos,
                        mLeftPaddingSize + mDayTimeBgWidth + mTimeBgBorderSize,
        ...
        if (isShowHour) {
            ...
            if (isShowTimeBgBorder) {
                canvas.drawRoundRect(mHourBgBorderRectF, mTimeBgBorderRadius, mTimeBgBorderRadius,
                    mTimeBgBorderPaint);
            }
            if (isDrawBg) {
        ...
        if (isShowMinute) {
            ...
            if (isShowTimeBgBorder) {
                canvas.drawRoundRect(mMinuteBgBorderRectF, mTimeBgBorderRadius, mTimeBgBorderRadius,
                    mTimeBgBorderPaint);
            }
            if (isDrawBg) {
                ...
                if (isShowTimeBgDivisionLine) {
                    ...
                    canvas.drawLine(mMinuteLeft + mTimeBgBorderSize, mTimeBgDivisionLineYPos,
                        mTimeBgSize + mMinuteLeft + mTimeBgBorderSize, mTimeBgDivisionLineYPos,
                        mTimeBgDivisionLinePaint);
                }
            }
            // draw minute text
        ...
        if (isShowSecond) {
            ...
            if (isShowTimeBgBorder) {
                canvas.drawRoundRect(mSecondBgBorderRectF, mTimeBgBorderRadius, mTimeBgBorderRadius,
                    mTimeBgBorderPaint);
            }
            if (isDrawBg) {
                ...
                if (isShowTimeBgDivisionLine) {
                    ...
                    canvas.drawLine(mSecondLeft + mTimeBgBorderSize, mTimeBgDivisionLineYPos,
                        mTimeBgSize + mSecondLeft + mTimeBgBorderSize, mTimeBgDivisionLineYPos,
                        mTimeBgDivisionLinePaint);
                }
            }
            // draw second text
