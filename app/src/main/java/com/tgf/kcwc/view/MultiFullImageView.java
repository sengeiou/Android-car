...
public class MultiFullImageView extends LinearLayout {
    ...
    public MultiFullImageView(Context context) {
        super(context);
        mContext = context;
        MAX_WIDTH = (int) DensityUtils.getPhonewidth(getContext());
    }

    public MultiFullImageView(Context context, AttributeSet attrs) {
        super(context, attrs);
        mContext = context;
        MAX_WIDTH = (int) DensityUtils.getPhonewidth(getContext());
    }
    ...
    private void initImageLayoutParams() {
        ...
        moreParaColumnFirst = new LayoutParams(pxMoreWandH, pxMoreWandH*3/4);
        morePara = new LayoutParams(pxMoreWandH, pxMoreWandH*3/4);
...
//        }
    ...
    private SimpleDraweeView createImageView(int position, final boolean isMultiImage) {
        final String photoInfo = imagesList.get(position);
        SimpleDraweeView imageView = new ColorFilterImageView(getContext());
        if (isMultiImage) {
            imageView.setScaleType(ScaleType.CENTER_CROP);
            imageView.setLayoutParams(position % MAX_PER_ROW_COUNT == 0 ? moreParaColumnFirst : morePara);
            Logger.d("isMultiImage");
        } else {
            Logger.d("NOMultiImage");
...
//        Glide.with(getContext()).load(photoInfo).diskCacheStrategy(DiskCacheStrategy.ALL)
//            .into(imageView);
...
//            .setControllerListener(controllerListener).setUri(Uri.parse(photoInfo)).build();
//        imageView.setController(controller);
        imageView.setImageURI(Uri.parse(photoInfo));
        return imageView;
    }
    ...
    private ImageLoadSuccess onImageLoadSuccess;

    public void setOnImageLoadSuccess(ImageLoadSuccess onImageLoadSuccess) {
