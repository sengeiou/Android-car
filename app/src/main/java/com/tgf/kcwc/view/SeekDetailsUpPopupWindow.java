...
public class SeekDetailsUpPopupWindow extends PopupWindow {

    private List<SeekTypeBean> mDataList;
    private Context mContext;
    private View catContentView;
    private Resources mRes;
    private ListView catPopListView;
    private WrapAdapter<SeekTypeBean> mCatgroyAdapter;
    private int selectPos;

    public SeekDetailsUpPopupWindow(Context context, List<SeekTypeBean> dataList) {
    ...
    public void setDeafultSelectPos(int pos) {
        selectPos = pos;
        SeekTypeBean selectItem = mDataList.get(selectPos);
        selectItem.isClick = true;
    ...
    public SeekTypeBean getSelectDataItem() {
    ...
    private SeekTypeBean mSelectDataItem;
    private void initAdapter() {
        mCatgroyAdapter = new WrapAdapter<SeekTypeBean>(mContext, mDataList, R.layout.item_listview_popwin) {
            @Override
        ...
        catPopListView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                IsSelc=true;
                SeekTypeBean selectItem = mDataList.get(position);
                selectItem.isClick = true;
                selectPos = position;
                mSelectDataItem = selectItem;
                singleChecked(mDataList, selectItem);
                mCatgroyAdapter.notifyDataSetChanged();
                SeekDetailsUpPopupWindow.this.dismiss();
            }
        });
    }

    private void initView() {
        catContentView = View.inflate(mContext, R.layout.seekdeselectpopupwind, null);
    ...
    private void singleChecked(List<SeekTypeBean> items, SeekTypeBean item) {
        for (SeekTypeBean d : items) {
            if (!d.name.equals(item.name)) {
                d.isClick = false;
            }
        }
    }
