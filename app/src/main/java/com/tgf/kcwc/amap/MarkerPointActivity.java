...
public class MarkerPointActivity extends BaseActivity implements LocationSource,
    ...
    private void initView() {
        ...
        searchText.addTextChangedListener(new TextWatcher() {
            ...
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                ...
                if (newText.length() > 0) {
                    InputtipsQuery inputquery = new InputtipsQuery(newText, "重庆");
                    Inputtips inputTips = new Inputtips(MarkerPointActivity.this, inputquery);
    ...
    protected void titleBarCallback(ImageButton back, FunctionView function, TextView text) {
        ...
        backEvent(back);
        text.setText("地图标点");
        function.setTextResource("确定", R.color.white, 15);
        function.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent = new Intent();
                if (mSavePoiItem != null) {
                    intent.putExtra(Constants.IntentParams.DATA, mSavePoiItem);

                }else {
                    if(firstItem!=null){
                        intent.putExtra(Constants.IntentParams.DATA, firstItem);
                    }
                }
    ...
    public void onRegeocodeSearched(RegeocodeResult result, int rCode) {
        ...
        if (rCode == AMapException.CODE_AMAP_SUCCESS) {
            ...
            CommonUtils.showToast(mContext, "error code is " + rCode);
        }
    }
            ...
            if (poiResult != null && poiResult.getQuery() != null) {
                if (poiResult.getQuery().equals(query)) {
                    ...
                    if (poiItems != null && poiItems.size() > 0) {
                        ...
                        CommonUtils.showToast(mContext, "无搜索结果");
                    }
                }
            } else {
                CommonUtils.showToast(mContext, "无搜索结果");
