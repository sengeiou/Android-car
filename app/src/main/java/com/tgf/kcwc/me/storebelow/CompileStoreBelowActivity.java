...
import com.tgf.kcwc.mvp.presenter.CompileStreBelowPresenter;
...
public class CompileStoreBelowActivity extends BaseActivity implements CompileStoreBelowView {

    private static final int IMAGE_PICKER = 1001;              //图片
    private FileUploadPresenter mFileUploadPresenter;                    //上传图片
    private CompileStreBelowPresenter mSponsorStreBelowPresenter;
    ...
    private CheckBox shopprice;                               //现车
    private CheckBox carstock;                                //展车
    ...
    protected void setUpViews() {
        ...
        mSponsorStreBelowPresenter = new CompileStreBelowPresenter();
        mSponsorStreBelowPresenter.attachView(this);
        initProgressDialog();

        mTitleBar = (RelativeLayout) findViewById(R.id.titleBar);
        carType = (LinearLayout) findViewById(R.id.car_type);
        type = (TextView) findViewById(R.id.type);
        shopprice = (CheckBox) findViewById(R.id.shopprice);
        carstock = (CheckBox) findViewById(R.id.carstock);
        ...
        mSponsorStreBelowPresenter.getDetail(IOUtils.getToken(mContext), ID + "");
    }
    ...
    public void onClick(View view) {
        ...
        switch (view.getId()) {
            case R.id.car_type:
                ...
                arg.put(Constants.IntentParams.IS_SEPBRAND, false);
                KPlayCarApp.putValue(Constants.IntentParams.SPONSORPLEASE, Constants.CarSeriesBack.COMPILESTOREBELOW_SUCCEED);
            ...
            case R.id.upholstery:
                ...
                if (mCarSelectBean.carId == 0) {
                    mSponsorStreBelowPresenter.getCarCategoryColors(mCarSelectBean.seriesId + "",
                            "car_series", "in");
                } else {
                    mSponsorStreBelowPresenter.getCarCategoryColors(mCarSelectBean.carId + "",
                            "car", "in");
            ...
            case R.id.upholsterylay:
                ...
                ImagePicker.getInstance().setMultiMode(false);
                intent = new Intent(this, ImageGridActivity.class);
                intent.putExtra(Constants.IntentParams.PHONETYPE, false);
                startActivityForResult(intent, IMAGE_PICKER);
                break;
    ...
    public void uploading() {
        ...
        if (!carstock.isChecked() && !shopprice.isChecked()) {
            CommonUtils.showToast(mContext, "请选择展车或现车");
            return;
        }
        ...
        storeBelowUpBean.diffConf = mEdtext.getText().toString().trim() + "";
        ...
        mSponsorStreBelowPresenter.getCreate(IOUtils.getToken(mContext), storeBelowUpBean);
    }
    ...
    @Override
    public void gainDataSucceed(CompileStoreBean baseBean) {
        CompileStoreBean.Detail detail = baseBean.data.detail;
        userlayout.setVisibility(View.VISIBLE);
        username.setText("上传人： " + detail.postUser.nickname);
        usertime.setText("上传时间： " + detail.postUser.postTime);
        type.setTextColor(getResources().getColor(R.color.text_color14));
        //type.setText(detail.carName);
/*        if (detail.carId != 0) {
            type.setText(detail.factoryName + " " + detail.seriesName + " " + detail.carName);
        } else {
            type.setText(detail.factoryName + " " + detail.seriesName + " 不限车型");
        }*/
        if (detail.carId != 0) {
            type.setText(detail.seriesName + " " + detail.carName);
        } else {
            type.setText(detail.seriesName + " 不限车型");
        }
        mCarSelectBean = new CarSelectBean();
        mCarSelectBean.name = detail.carName;
        mCarSelectBean.carId = detail.carId;
        mCarSelectBean.seriesId = detail.seriesId;
        mCarSelectBean.mFirmId = detail.factoryId;
        if (detail.typeShow == 1) {
            shopprice.setChecked(true);
        } else {
            shopprice.setChecked(false);
        }
        if (detail.typeExist == 1) {
            carstock.setChecked(true);
        } else {
            carstock.setChecked(false);
        }
        ...
        if (TextUtil.isEmpty(detail.appearanceColorName)) {
            mFacadename.setText("全部");
        } else {
        ...
        if (TextUtil.isEmpty(detail.interiorColorName)) {
            mUpholsteryname.setText("全部");
        } else {
        ...
        mEdtext.setText(detail.diffConf);
    }
    ...
    public void dataColourSucceed(List<CarColor> datalist) {
        ...
        mDropUpColourPopupWindow.show(CompileStoreBelowActivity.this);
