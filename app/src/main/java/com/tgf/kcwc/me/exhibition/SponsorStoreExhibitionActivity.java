...
public class SponsorStoreExhibitionActivity extends BaseActivity implements SponsorStoreExhibitionView {
    ...
    private SponsorStreExhibitionPresenter mSponsorStreExhibitionPresenter;

    private RelativeLayout mTitleBar;
    private TextView title;                                 //展会标题
    private LinearLayout carType;                                 //车系类型
    private TextView type;                                    //车系类型名字
    private TextView hinttext;                                    //提示
    ...
    private String ExhibitionName = "";                //展会名字

    @Override
    protected void setUpViews() {
        mFileUploadPresenter = new FileUploadPresenter();
        mFileUploadPresenter.attachView(uploadView);
        mSponsorStreExhibitionPresenter = new SponsorStreExhibitionPresenter();
        mSponsorStreExhibitionPresenter.attachView(this);
        initProgressDialog();
        ...
        mSponsorStreExhibitionPresenter.getAdd(IOUtils.getToken(mContext));
        title.setText(ExhibitionName);
    }
    ...
    protected void titleBarCallback(ImageButton back, FunctionView function, TextView text) {
        ...
        backEvent(back);
        text.setText(R.string.sponsorstoreexhibition);
        text.setTextSize(16);
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        ExhibitionID = getIntent().getStringExtra(Constants.IntentParams.DATA);
        ExhibitionName = getIntent().getStringExtra(Constants.IntentParams.DATA2);
    ...
    public void onClick(View view) {
        ...
        switch (view.getId()) {
            case R.id.car_type:
                ...
                if (!TextUtil.isEmpty(ExhibitionID)){
                    arg.put(Constants.IntentParams.EVENT_ID, ExhibitionID);
                    KPlayCarApp.putValue(Constants.IntentParams.SPONSORPLEASE, Constants.CarSeriesBack.SPONSORSTOREEXHIBITION_SUCCEED);
                    CommonUtils.startNewActivity(this, arg, StoreBrandListsActivity.class);
                }else {
                    CommonUtils.showToast(mContext, "没有获取到展会ID");
                }
                break;
            case R.id.upholstery:
                ...
                if (mCarSelectBean.carId == 0) {
                    mSponsorStreExhibitionPresenter.getCarCategoryColors(mCarSelectBean.seriesId + "",
                            "car_series", "in");
                } else {
                    mSponsorStreExhibitionPresenter.getCarCategoryColors(mCarSelectBean.carId + "",
                            "car", "in");
    ...
    public void uploading() {
        ...
        mSponsorStreExhibitionPresenter.getCreate(IOUtils.getToken(mContext), storeBelowUpBean);
    }
    ...
    public void dataColourSucceed(List<CarColor> datalist) {
        ...
        mDropUpColourPopupWindow.show(SponsorStoreExhibitionActivity.this);
    ...
    public void dataJurisdictionSucceed(StorJurisdictionBean storJurisdictionBean) {
        intExtra = storJurisdictionBean.data.auth;
    }
