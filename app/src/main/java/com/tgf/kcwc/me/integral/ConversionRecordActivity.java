...
public class ConversionRecordActivity extends BaseActivity implements IntegralRecordView {
    ...
    private WrapAdapter<IntegralRecordListBean.DataList> mAdapter;
    private List<IntegralRecordListBean.DataList> datalist = new ArrayList<>();
    int page = 1;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setmAdapter();
        mIntegralRecordPresenter.getPointsReceiveList(IOUtils.getToken(mContext), page);
        mListView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
            public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {

                mIntegralRecordPresenter.getRecordDetail(IOUtils.getToken(mContext), datalist.get(i).id + "");
            }
        });
    }
    ...
    public void setmAdapter() {
        mAdapter = new WrapAdapter<IntegralRecordListBean.DataList>(mContext, R.layout.integral_recordlistview_item, datalist) {
            @Override
            public void convert(ViewHolder helper, IntegralRecordListBean.DataList item) {
                ...
                int num = item.num;
                if (num <= 1) {
                    mTitle.setText(item.productName);
                } else {
                    mTitle.setText(item.productName + "×" + num);
                }
                mTime.setText(item.createTime);
                mBonuspoint.setText("-" + item.usePoints + "分");
