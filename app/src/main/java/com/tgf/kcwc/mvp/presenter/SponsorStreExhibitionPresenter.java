...
public class SponsorStreExhibitionPresenter extends WrapPresenter<SponsorStoreExhibitionView> {

    private SponsorStoreExhibitionView mView;
    private StoreExhibitionService mService;

    @Override
    public void attachView(SponsorStoreExhibitionView view) {
        mView = view;
    ...
    public void getAdd(String token) {
        RXUtil.execute(mService.getAdd(token),
            new Observer<StorJurisdictionBean>() {
                @Override
                public void onCompleted() {
                    mView.setLoadingIndicator(false);
                }
                ...
                @Override
                public void onNext(StorJurisdictionBean storeBelowBean) {
                    if (storeBelowBean.code == 0) {
                        mView.dataJurisdictionSucceed(storeBelowBean);
                    } else {
    ...
    public void getCreate(String token, StoreBelowUpBean storeBelowUpBean) {
        RXUtil.execute(mService.getCreate(token, storeBelowUpBean.appearanceColorName,
            ...
            storeBelowUpBean.status), new Observer<BaseBean>() {
                @Override
                public void onCompleted() {
                    mView.setLoadingIndicator(false);
                }
