...
import com.tgf.kcwc.mvp.presenter.CompilePleasePresenter;
...
public class CompilePleasePlayActivity extends BaseActivity implements CompilePleaseView {

    private FileUploadPresenter mFileUploadPresenter;                           //上传图片
    private CompilePleasePresenter mCompilePleasePresenter;
    ...
    CompilePleaseBean mCompilePleaseBean;
    ...
    public void gainTypeDataList() {
        ...
        mTypeDataItem.title = "test_drive";
    }
    ...
    protected void onNewIntent(Intent intent) {
        ...
        if ((Intent.FLAG_ACTIVITY_CLEAR_TOP & intent.getFlags()) != 0) {
            int intExtra = intent.getIntExtra(Constants.IntentParams.ID, -1);
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mCompilePleasePresenter = new CompilePleasePresenter();
        mCompilePleasePresenter.attachView(this);
        ...
        mCompilePleasePresenter.getEdit(IOUtils.getToken(mContext), ID);
    }
    ...
    public void onClick(View view) {
        ...
        switch (view.getId()) {
            ...
            case R.id.scene_type: //选择类型
                ...
                mDropUpPopupWindow.show(CompilePleasePlayActivity.this);
            ...
            case R.id.drivingtestlayout: //试乘试驾
                ...
                KPlayCarApp.putValue(Constants.IntentParams.SPONSORPLEASE, Constants.CarSeriesBack.COMPILEPLEASEPLAY_SUCCEED);
    ...
    public void Confirm() {
        ...
        if (mTypeDataItem.id == 2) {
            mCompilePleasePresenter.getCreate(builderParams(car_id));
        } else {
            mCompilePleasePresenter.getCreate(builderParams1(car_id));
        }
    ...
    private Map<String, String> builderParams(String car_id) {
        ...
        params.put("longitude", mCompilePleaseBean.data.longitude);
        params.put("latitude", mCompilePleaseBean.data.latitude);
        params.put("local_address", mCompilePleaseBean.data.localAddress);
        params.put("adcode", mCompilePleaseBean.data.adcode);
    ...
    protected void onDestroy() {
        ...
        if (mCompilePleasePresenter != null) {
            mCompilePleasePresenter.detachView();
        }
    }
    ...
    @Override
    public void dataSucceed(CompilePleaseBean compilePleaseBean) {
        mCompilePleaseBean = compilePleaseBean;
        CompilePleaseBean.Data data = compilePleaseBean.data;
        if (data.cover != null) {
            mHintpicture.setVisibility(View.GONE);
            mDrivdetailslay.setVisibility(View.VISIBLE);
        ...
        if (compilePleaseBean.data.type.equals("test_drive")) {
            mTypeDataItem = new DataItem();
            mTypeDataItem.id = 1;
            mTypeDataItem.name = "试乘试驾";
            mTypeDataItem.title = "test_drive";
            mType.setText(mTypeDataItem.name);
            setTypeLay(mTypeDataItem.id);
        } else if (compilePleaseBean.data.type.equals("self_drive")) {
            mTypeDataItem = new DataItem();
            mTypeDataItem.id = 2;
            mTypeDataItem.name = "车主自驾";
            mTypeDataItem.title = "self_drive";
            mType.setText(mTypeDataItem.name);
            setTypeLay(mTypeDataItem.id);
        } else if (compilePleaseBean.data.type.equals("other")) {
            mTypeDataItem = new DataItem();
            mTypeDataItem.id = 3;
            mTypeDataItem.name = "其它";
            mTypeDataItem.title = "other";
            mType.setText(mTypeDataItem.name);
            setTypeLay(mTypeDataItem.id);
        } else {
        ...
        gatherTip = new Tip();
        gatherTip.setAdcode(data.rendAdcode);
        gatherTip.setDistrict(data.rendezvous);
        gatherTip.setPostion(new LatLonPoint(data.rendLatitude, data.rendLongitude));
        gatherCity = data.rendCity;
        ...
        List<CompilePleaseBean.TopicList> topicList = data.topicList;
        for (CompilePleaseBean.TopicList topicbean : topicList) {
            Topic topic = new Topic();
        ...
        mExplainAdapter.notifyDataSetChanged();
        List<CompilePleaseBean.CarList> carList = data.carList;
        if (carList != null || carList.size() != 0) {
            for (CompilePleaseBean.CarList carList1 : carList) {
                CarSelectBean carSelectBean = new CarSelectBean();
                carSelectBean.name = carList1.carName;
                carSelectBean.carId = carList1.id;
                mCarSelectBeen.add(carSelectBean);
            }
            mCarSelectBeanAdapter.notifyDataSetChanged();
        }
