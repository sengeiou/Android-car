...
public class StoreBrandListsActivity extends BaseActivity

        implements OnItemClickListener<Brand>, BrandDataView {
    private static final String TAG = StoreBrandListsActivity.class
            .getSimpleName();
    ...
    private boolean isSepBrand = false;//false是店内车 true是展会车
    private boolean isUnlimited = false; //是否拥有不限车型
    private String mEveId = ""; //展会ID
    ...
    protected void setUpViews() {
        ...
        if (isSepBrand) {
            brandListPresenter.getExhibitionaddbrandList(IOUtils.getToken(mContext), mEveId);
        } else {
            brandListPresenter.getaddbrandList(IOUtils.getToken(mContext));
        }
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        isUnlimited = intent.getBooleanExtra(Constants.IntentParams.ISUNLIMITED, false);
        mEveId = intent.getStringExtra(Constants.IntentParams.EVENT_ID);
    ...
    public void onItemClick(ViewGroup parent, View view, Brand item, int position) {
        ...
        if (sponsor.equals(Constants.CarSeriesBack.SPONSORSTOREEXHIBITION_SUCCEED) || sponsor.equals(Constants.CarSeriesBack.COMPILESTOREEXHIBITION_SUCCEED)) {
            KPlayCarApp.putValue(Constants.CarSeriesBack.HALLID, item.hallId);
            KPlayCarApp.putValue(Constants.CarSeriesBack.BOOTHID, item.boothId);
        }
        Map<String, Serializable> args = new HashMap<String, Serializable>();
        args.put(Constants.IntentParams.ID, item.brandId);
        args.put(Constants.IntentParams.DATA, item.brandName);
        CommonUtils.startNewActivity(this, args, SelectSeriesActivity.class);
    ...
    public void showData(List<Brand> data) {
        ...
        mRv.addItemDecoration(new DividerItemDecoration(StoreBrandListsActivity.this,
