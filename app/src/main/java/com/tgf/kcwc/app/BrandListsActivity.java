 ...
 * 品牌列表 可多选，可单选（单选会具体到车型），根据Intent传递的参数进行区分
 * 一级品牌
 */
...
public class BrandListsActivity extends BaseActivity
    ...
    private static final String TAG = BrandListsActivity.class
            .getSimpleName();
...
//        if (isSepBrand) {
//            brandListPresenter.loadSepBrandsDatas(mSenseId + "", "0");
//        } else {
//            brandListPresenter.loadBrandsDatas("car");
//        }
        brandListPresenter.getBrandsDatas(IOUtils.getToken(mContext), "0");
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        isSepBrand = intent.getBooleanExtra(Constants.IntentParams.IS_SEPBRAND, false);
        mSenseId = intent.getIntExtra(Constants.IntentParams.EVENT_ID, -1);
        mIntentType = intent.getIntExtra(Constants.IntentParams.INTENT_TYPE, -1);
        isJump = intent.getBooleanExtra(Constants.IntentParams.IS_JUMP, false);
        isSelectModel = intent.getBooleanExtra(Constants.IntentParams.TITLE, false);
        mIndex = intent.getIntExtra(Constants.IntentParams.INDEX, -1);
        setContentView(R.layout.activity_brand_list);

        if (Constants.IntentParams.SINGLE_BRAND_TYPE == mIntentType) {
            mConfirmBtn.setVisibility(View.GONE);
        } else if (Constants.IntentParams.MUlTI_BRAND_TYPE == mIntentType) {
    ...
    public void onItemClick(ViewGroup parent, View view, Brand item, int position) {
        ...
        if (Constants.IntentParams.MUlTI_BRAND_TYPE == mIntentType) {
            if (item.isSelected) {
                item.isSelected = false;
            } else {
                item.isSelected = true;
            }
        } else if (Constants.IntentParams.SINGLE_BRAND_TYPE == mIntentType) {

            if (isJump) {
                Map<String, Serializable> args = new HashMap<String, Serializable>();
                args.put(Constants.IntentParams.ID, item.brandId);
                args.put(Constants.IntentParams.DATA, item.brandName);
                args.put(Constants.IntentParams.TITLE, isSelectModel);
                args.put(Constants.IntentParams.INDEX, mIndex);
                CommonUtils.startNewActivity(this, args, SelectFactorySeriesActivity.class);
            } else {
    ...
    public void showData(List<Brand> data) {
        ...
        if (StringUtils.checkNull(sponsor)) {
            if (sponsor.equals(Constants.CarSeriesBack.SPONSORPLEASEPLAY_SUCCEED) || sponsor.equals(Constants.CarSeriesBack.COMPILEPLEASEPLAY_SUCCEED)) {
                for (int i = 0; i < datalist.size(); i++) {
                    if (datalist.get(i).brandId == 0) {
                        datalist.remove(i);
                    }
                }
            }
        ...
        mRv.addItemDecoration(new DividerItemDecoration(BrandListsActivity.this,
